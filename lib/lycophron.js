 ...
 * @constructor
 */
module.exports.Dictionary = function (name, useAgent, url) {
    'use strict';

    var self = this,

        localeInfo = require('../locale/info.json'),
...
module.exports.Dictionary.processWords = function (words, letters) {
    'use strict';
    ...
    for (i = 0; i < words.length; i += 1) {
        ...
        if (shouldSkip) {
            console.warn('Skipped (contains letter that is not a consonant nor a vowel or capitalized): ' + words[i]);
            continue;
        }

        encodedWords = module.exports.Dictionary.encodeWord(normalized, letters.encodeMap);
    ...
    console.log(dict.numWords, dict.numUniqueWords);

    // translates words into a special dictionary format
    return dict;
};

module.exports.Dictionary.encodeWord = function (word, encodeMap) {
    'use strict';
